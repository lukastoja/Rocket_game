	...
	if (World)
	{
		...
		if (m > 8)
		{
			...
			if (tunel)
			{
				...
				/*double x = NextSpawnPoint.GetLocation().X;
				double y = NextSpawnPoint.GetLocation().Y;
				double z = NextSpawnPoint.GetLocation().Z;

				NextSpawnPoint = FTransform(FVector(x, y, z));*/
			}
